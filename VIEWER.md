...
**V1 — static architecture viewer.** Vite + React + react-three-fiber. Load a spec (file or paste), render the lattice, step through layers, animate the sweep. Rank 1–3 literal, 4–5 stacked. No Python server yet — it reads a JSON file, which makes it developable and testable entirely standalone. *Built:* `viewer/` — `npm install && npm run dev`. Five bundled sample specs, regenerated by `python viewer/make_samples.py` rather than being artifacts with no recipe (2-D sparse LSTM, 3-D paired-schedule Mamba-ND, 4-D S4D exercising the stacking, a CaFA hybrid, and a ViT whose patch grid is one joint sequence); a time sweep breathes uniformly (no lattice direction to draw), the kernel family flashes all axes at once rather than pretending to sweep (§7 Q1 answered by idiom, not schema), and the never-swept-axes warning renders red in the sidebar. One hard-won rule: the WebGL canvas is never remounted — swap specs by rebuilding the instanced mesh and recentering the camera, or the context is lost and the scene goes permanently black.
...
1. ~~**Does the spec need to describe the kernel family?**~~ **Answered — yes, and answering it "by idiom" instead cost a shipped bug.** The original decision was to leave the schema alone and let the renderer special-case the kernel family. The renderer did, correctly. The *document* meanwhile went on claiming one spatial sweep per layer — sweeps that never happen — so the sidebar, which reads the document rather than the special case, printed them, and any other consumer of the spec would have believed them too. Spec v2 gives layers a `kind`, the `axes` they actually mix, and `contracted`; `nd_method.family` is derived rather than hardcoded. The general lesson is in DEBUG.md #26: **a consumer written to compensate for its producer is a bug report about the producer.**
2. ~~**How is a hybrid `nd_method` shown?**~~ **Answered.** A hybrid layer names its contracted axes and its swept one separately — `station ⊗ sensor + time →` — because that is what it does. The scene flashes the lattice rather than drawing a wavefront, and the arrow is hidden. The joint (`flatten`) family renders the same way in a different colour, with the token count instead of a direction.
