...
try:  # torch-dimensions patch: Triton has no Metal backend and needs CUDA; the stubs
    import triton  # torch-dimensions patch: let this module import, so rms_norm_ref below is usable
    import triton.language as tl  # torch-dimensions patch
    _HAS_TRITON = True  # torch-dimensions patch
except ImportError:  # torch-dimensions patch
    from torch_dimensions.mixers.mamba2_compat import triton_stub  # torch-dimensions patch

    triton, tl = triton_stub()  # torch-dimensions patch
    _HAS_TRITON = False  # torch-dimensions patch
