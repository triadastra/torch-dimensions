...
from .layernorm_gated import RMSNorm as RMSNormGated  # torch-dimensions patch: vendored path
...
# torch-dimensions patch: the SISO scan exists upstream only as Triton, which
# torch-dimensions patch: needs CUDA. This dispatcher calls the real kernel when the
# torch-dimensions patch: tensors are on CUDA and it imports, and our torch transcription
# torch-dimensions patch: of the same recurrence otherwise. See mixers/mamba3_compat.py,
# torch-dimensions patch: which is explicit that those numerics are ours, not the authors'.
from torch_dimensions.mixers.mamba3_compat import mamba3_siso_combined  # torch-dimensions patch

try:  # torch-dimensions patch: inference-step rotary; the step path needs CUDA regardless
    ...
    apply_rotary_qk_inference_fwd = None  # torch-dimensions patch
