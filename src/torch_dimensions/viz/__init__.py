...
from torch_dimensions.viz.weights import weights

__all__ = ["BUNDLE", "bundle_exists", "resolve_spec", "serve", "show", "weights"]
...
class _Handler(SimpleHTTPRequestHandler):
    ...
    def __init__(self, *args, payload: bytes, weights_payload: bytes = b"", **kwargs) -> None:
        self._payload = payload
        self._weights = weights_payload
    ...
    def do_GET(self) -> None:
        ...
        if route == "/spec.json":
            self._json(self._payload)
            return
        # Absent when the target was a spec dict or a .json file rather than a
        # model: there are no parameters to serve, and 404 says so honestly.
        if route == "/weights.json":
            if not self._weights:
                self.send_error(404, "no weights: the viewer was given a spec, not a model")
                return
            self._json(self._weights)
            return
        super().do_GET()
...
def serve(target: Any, port: int = 0) -> ThreadingHTTPServer:
    ...
    weights_payload = b""
    if isinstance(target, nn.Module):
        weights_payload = json.dumps(weights(target)).encode()
    server = ThreadingHTTPServer(
        ("127.0.0.1", port),
        partial(_Handler, payload=payload, weights_payload=weights_payload),
    )
