...
**Status:** Phases 0–5 built (`Lattice`, `ScanPlan`, `AxialScan`, `LSTM`/`GRU`, conformance suite, data layer), Phase 6 partially (kernel contraction), Phase 7's portable core built (`S4D`/`Mamba` — pure-torch mixers cross-validated against the upstream reference kernels, running on CPU/CUDA/MPS). Remaining: kernel-family strategies (`td.axial_attention`, `td.cafa`), fused-kernel fast paths, registry/config, viewer. See [PLAN.md](PLAN.md) for build order.
